...
`source = hermes://agent/tonnybox-pm`. Inbound commands arrive through the
...
board, and route work to other agents (e.g. the `tonnybox-dev` role).
...
Put `repo = tonnybox` in event data; never insert repo or agent
